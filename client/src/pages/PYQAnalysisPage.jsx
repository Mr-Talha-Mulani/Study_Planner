...
import { MOCK_PYQ_ANALYSIS, MOCK_SUBJECTS } from '../utils/mockData'
import { RadarChart, PolarGrid, PolarAngleAxis, Radar, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, Tooltip } from 'recharts'
import toast from 'react-hot-toast'

export default function PYQAnalysisPage() {
  const [analysis, setAnalysis] = useState(MOCK_PYQ_ANALYSIS)
  ...
  const analyzeFile = async () => {
    ...
    setAnalysis(prev => prev.map(item => ({
      ...item,
      frequency: item.frequency + Math.floor(Math.random() * 3),
      importanceScore: Math.min(10, item.importanceScore + Math.floor(Math.random() * 2))
    })).sort((a, b) => b.frequency - a.frequency))

    toast.success('✅ PYQ analysis complete! Importance scores updated.')
    setLoading(false)
  }
  ...
  return (
    <div className="page-container fade-in">
      ...
      <div className="alert alert-warning mb-6">
        <span>⚠️</span>
        <div>
          <div className="font-semibold">Accuracy Disclaimer</div>
          <div style={{ fontSize: '0.8rem', marginTop: '4px' }}>
            AI-based PYQ mapping accuracy is estimated at 70-85%. Always verify with your teacher. Use this as a guide to prioritize topics, not as a guarantee of what will appear in the exam.
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        <div className="card">
          ...
          >
            {loading ? (
              <><span className="spinner" style={{ width: 16, height: 16, borderWidth: 2 }} /> AI Analyzing...</>
            ) : '🤖 Analyze with AI'}
          </button>
        </div>
      ...
      <div className="card mb-6">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={chartData} margin={{ top: 0, right: 20, left: 0, bottom: 60 }}>
            ...
            <Tooltip
              contentStyle={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: '8px', color: 'var(--text-primary)', fontSize: '0.8rem' }}
            />
          ...
          {analysis.sort((a, b) => b.frequency - a.frequency).map((item, idx) => (
            <div
              ...
              style={{
                ...
                background: idx < 3 ? 'hsl(350,80%,58%,0.06)' : 'var(--bg-surface2)',
                border: `1px solid ${idx < 3 ? 'hsl(350,80%,58%,0.2)' : 'var(--border-subtle)'}`
              }}
            >
              {/* Rank */}
              <div style={{
                width: 28, height: 28, borderRadius: '50%',
                background: idx === 0 ? 'linear-gradient(135deg, hsl(48,100%,50%), hsl(38,100%,55%))' :
                             idx === 1 ? 'linear-gradient(135deg, hsl(220,10%,75%), hsl(220,10%,85%))' :
                             idx === 2 ? 'linear-gradient(135deg, hsl(25,80%,50%), hsl(25,80%,65%))' : 'var(--bg-surface3)',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontWeight: 800, fontSize: '0.8rem',
                color: idx < 3 ? (idx === 1 ? 'hsl(220,10%,30%)' : 'hsl(38,80%,25%)') : 'var(--text-muted)',
                flexShrink: 0
              }}>
                {idx + 1}
              </div>
              ...
              <div style={{ minWidth: 100 }}>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className="text-xs font-bold" style={{ color: item.importanceScore >= 8 ? 'hsl(350,80%,72%)' : 'hsl(38,92%,65%)' }}>
                ...
                <div className="progress-bar-container" style={{ height: 5 }}>
                  <div
                    ...
                    style={{
                      width: `${item.importanceScore * 10}%`,
                      background: item.importanceScore >= 8 ? 'var(--grad-danger)' : 'var(--grad-primary)'
                    }}
                  />
                </div>
              </div>
